...
class bse:
    def __init__(self, space_input, space_output, test_x):
    ...
    def set_test(self, x_test, y_test=None):
        self.test_y = y_test
        self.test_x = x_test
        self.Ntest = len(x_test)
        self.test_time = np.linspace(np.min(self.test_x), np.max(self.test_x), 500)
    ...
    def compute_test_moments(self):
        cov_space = Spec_Mix(self.x,self.x,self.gamma,self.theta,self.sigma) + 1e-5*np.eye(self.Nx) + self.sigma_n**2*np.eye(self.Nx)
        cov_time = Spec_Mix(self.test_x,self.test_x, self.gamma, self.theta, self.sigma)
        cov_star = Spec_Mix(self.test_x,self.x, self.gamma, self.theta, self.sigma)
        self.test_post_mean = np.squeeze(cov_star@np.linalg.solve(cov_space,self.y))
        self.test_post_cov = cov_time - (cov_star@np.linalg.solve(cov_space,cov_star.T))
    ...
    def plot_test_posterior(self, flag=None):
        #posterior moments for time
        plt.figure(figsize=(18,6))
        if self.test_y is not None:
            plt.plot(self.test_x,self.test_y,'.r', label='observations')
        plt.plot(self.test_x,self.test_post_mean, color='blue', label='posterior mean')
        error_bars = 2 * np.sqrt(np.diag(self.test_post_cov))
        plt.fill_between(self.test_x, self.test_post_mean - error_bars, self.test_post_mean + error_bars, color='blue',alpha=0.1, label='95% error bars')
        if flag == 'with_window':
            plt.plot(self.test_x, 2*self.sigma*np.exp(-self.alpha*self.test_x**2))
        plt.title('Observations and posterior interpolation')
        plt.xlabel(self.time_label)
        plt.ylabel(self.signal_label)
        plt.legend()
        plt.xlim([min(self.test_x),max(self.test_x)])
        plt.tight_layout()
        plt.show()
